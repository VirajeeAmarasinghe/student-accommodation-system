...
<html>
    ...
    <body>
        ...
        <%
            ...
            ArrayList<Object> propertyList = null;
            try {
                if (request.getParameter("btnSearch") == null) {
                    propertyList = propDAO.getAll();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
                ...
                <ul>
                    ...
                    <li>Type
                        <select name="typeProperty" class="fields">
                            ...
                            <option value="<%=p.get(i).getT_id()%>" name="typeOption">
                                <%=p.get(i).getTypeName()%>
                            </option>
                            <%}%>
                        </select> 
        ...
        <form action="DisplayProperty" method="post"> 
            <%
                ...
                for (int i = 0; i < propertyList.size(); i++) {
                    ...
                    String add = "Location : " + prop.getAddNo() + prop.getAddStreet() + prop.getAddCity();
                    String src = "DisplayImage?ID=" + prop.getPropID();
                    Object ob = pTypeDAO.get(prop.gettID());
                    PropertyType pType = (PropertyType) ob;
            %>
            <div class="product">
                <div class="image">                    
                    <img src="<%=src%>" alt="Produce Image" width="360" height="238">
                </div>
                <input type="hidden" name="code[]" value="<%= prop.getPropID()%>" >
                <div class="text">
                    <label class="pro_name"><%=pType.getTypeName()%></label><br>
                    <label class="pro_name"><%=add%></label><br>
                    <label class="price">Rent :  ‎£<%= prop.getRent()%></label><br>
                    <div class="buttonSections">
                        <div class="view" id="viewMore">
                            <button type="submit" name="viewMore" value="<%=prop.getPropID()%>" class="buttons">View more</button>
                        </div>	
                        <div class="buy" id="buy">
                            <button type="submit" name="rent" value="<%=prop.getPropID()%>" id="rent" class="buttons">Rent Now</button>
                        </div>
                        <div class="view" id="viewProp">
                            <button type="submit" name="view" value="<%=prop.getPropID()%>" id="rent" class="buttons">Request View</button>
                        </div>
                    </div>
                </div>            
            </div>
